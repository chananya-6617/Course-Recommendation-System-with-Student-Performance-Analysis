...
% Enrollment facts: enrolled(StudentID, CourseID, Grade, Score)
enrolled('S001', 'C001', 'A', 92).
enrolled('S001', 'C002', 'B+', 85).
enrolled('S001', 'C003', 'A-', 89).
enrolled('S002', 'C001', 'B+', 82).
enrolled('S002', 'C004', 'B', 76).
enrolled('S002', 'C005', 'A-', 88).
enrolled('S003', 'C006', 'A', 94).
enrolled('S003', 'C007', 'B+', 86).
enrolled('S003', 'C003', 'A', 91).
enrolled('S004', 'C008', 'B', 79).
enrolled('S004', 'C009', 'B+', 84).
enrolled('S005', 'C001', 'C+', 68).
enrolled('S005', 'C004', 'B-', 74).
enrolled('S006', 'C006', 'A-', 87).
enrolled('S006', 'C007', 'B+', 83).
enrolled('S006', 'C003', 'A', 90).

% ============ PROLOG RULES FOR RECOMMENDATIONS ============

% Rule 1: Check if student has completed prerequisites
completed_prerequisites(StudentID, CourseID) :-
    \+ prerequisite(CourseID, _),  % No prerequisites
    !.
completed_prerequisites(StudentID, CourseID) :-
...
recommend_by_department(StudentID, CourseID) :-
    ...
    \+ enrolled(StudentID, CourseID, _, _).
...
% Rule 5: Overall recommendation (combining all factors)
...
% Rule 7: Check if student is eligible for advanced courses
eligible_for_advanced(StudentID) :-
    student(StudentID, _, _, GPA, _),
    GPA >= 8.5,
    findall(Score, enrolled(StudentID, _, _, Score), Scores),
    average(Scores, Avg),
    Avg >= 85.
...
% Rule 8: Get student performance level
...
% Rule 9: Find similar courses based on department and difficulty
similar_course(CourseID, SimilarID) :-
    course(CourseID, _, Dept, _, Difficulty, _),
    course(SimilarID, _, Dept, _, Difficulty, _),
    CourseID \= SimilarID.

% Rule 10: Get course popularity rank
popularity_rank(CourseID, Rank) :-
    findall(Pop, course(_, _, _, _, _, Pop), AllPops),
    sort(0, @>=, AllPops, SortedPops),
    nth1(Rank, SortedPops, Pop),
    course(CourseID, _, _, _, _, Pop).